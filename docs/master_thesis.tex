...
\date{Last modified on 2024-04-02}
...
applied to sports. Teams use data to analyze player performance, identify strengths and weaknesses, and develop strategies to win games \citep{2}. Basketball Analytics can provide qualitative analysis to team owners, players, coaches, and technical staff to help them predict future situations and make informed decisions to improve performance.
...
At its core, Pitch Control model provides a quantitative framework using tracking data to assess player movement, spatial control, and decision making on the soccer field. Based on such a framework, we introduced the concept of \emph{Offside Control} to analyze the performance of players with respect to the danger created behind the offside line of the opposing team.
...
\begin{table}[!h]
\centering
\caption{\label{tab:ranking-tab}Average Effective Offside Control values and Efficiency for each team analyzed}
\centering
\resizebox{\ifdim\width>\linewidth\linewidth\else\width\fi}{!}{
\begin{tabular}[t]{lrr}
\toprule
